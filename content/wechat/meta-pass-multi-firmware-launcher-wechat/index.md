---
title: "孩子的 AI 玩具一次只能玩一个？2 天做出多固件启动器和下期预告"
date: 2026-09-13T08:00:00+08:00
slug: "meta-pass-multi-firmware-launcher-wechat"
draft: true
description: "FoloToy AI Passport 的 plays 市场里固件很多，但设备一次只能刷一个，想换着玩就得整片重刷。我花两天写了多固件启动器 meta-pass：两个槽位、两条安装通道、三个被硬件约束逼出来的设计。"
tags: ["AI", "ESP32", "固件", "FoloToy", "AI Passport", "硬件", "复盘"]
categories: ["wechat"]
image: cover.jpg
---

孩子的玩具一次只能装一个玩法，换个玩法就得连电脑重新刷机，这事儿你遇到过吗？
...
虽然全程AI护航，但毕竟是自己第一次做 SOC 的开发，学到了不少新东西，所以记录一下。这篇复盘会讲到：两个槽位、两条安装通道、三个被约束逼出来的设计、两个坑，最后是我的两点心得。
...
今天就到这里。你给孩子或者自己刷固件的时候，踩过什么坑？对「约束就是设计的来源」这个说法有不同意见的，也欢迎来辩。期待看到你的留言。

下一篇预告：从两个槽位扩成三个，启动器从 3MB 瘦身到 1.44MB 的过程中，暴露了一个潜伏的偏移 bug，名字差点写进隔壁分区。

---

**参考链接**（微信不支持外链跳转，复制到浏览器打开）：

1. FoloToy AI Passport plays 市场
   https://ai-passport.folotoy.cn
2. ESP-IDF OTA API 文档（含回滚机制）
   https://docs.espressif.com/projects/esp-idf/en/latest/esp32c3/api-reference/system/ota.html
3. MDN：Web Serial API（要求安全上下文）
   https://developer.mozilla.org/en-US/docs/Web/API/Web_Serial_API
4. espressif/esptool-js：esptool 的 JavaScript 实现
   https://github.com/espressif/esptool-js
5. ESP32-C3 技术规格书（Strapping 引脚）
   https://www.espressif.com/sites/default/files/documentation/esp32-c3_datasheet_en.pdf
6. meta-pass 项目源码（MIT，含设计文档与决策记录）
   https://github.com/alexwwang/meta-pass
7. 本文博客完整版（中英文双语）
   https://blog.chuanxilu.net/posts/2026/09/meta-pass-multi-firmware-launcher/

---

## 编辑备注（发布前删除此节）

- [ ] 标题 30 字（含标点，AI 按 2 字符计），≤32 字限制；含数字 2；读者利益前置（孩子的玩具），含「下期预告」结构承诺，无震惊体
- [ ] 摘要 100 字，符合 120 字限制，含核心关键词 固件 / FoloToy / meta-pass
- [ ] 正文与博客中文版对齐，无删减；格式适配：内联代码词（esp_ota_set_boot_partition、write_flash 等）微信不渲染等宽字体，已去反引号保持原样；无表格无需转换
- [ ] 钩子按惯例不剧透具体数字，开篇场景与博客版一致
- [ ] 按既定要求：正文不做引流截断，全文完整发布；「阅读原文」设置为博客原文 tracking URL：
      https://blog.chuanxilu.net/posts/2026/09/meta-pass-multi-firmware-launcher/?utm_source=wechat&utm_medium=article&utm_campaign=meta-pass-multi-firmware-launcher
- [x] 图片已齐：cover.jpg（Gemini 生成，16:9，上传素材库做头图时注意裁切区域为中间 383×383 方形）+ 5 张文内图（illustration-1 为 Gemini 生成的双通道概念图；illustration-2~5 为项目 v0.1 时代真实截图，取自 meta-pass 仓库 commit 54e6475：USB 安装页、槽位详情、未签名警告、启动器菜单；repo 当前的安装页和警告页截图是 v0.2 的，勿用）。粘贴正文时图片需逐张手动上传微信编辑器
- [ ] 发布前用预览模式检查排版，深色模式过一遍
- [ ] 话题标签建议：#AI玩具 #固件 #ESP32；合集：AI应用开发
