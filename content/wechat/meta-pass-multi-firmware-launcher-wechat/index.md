...
第一条是免线缆的：设备开 Wi-Fi 热点（SoftAP），屏幕显示随机密码和一个 6 位一次性配对码，手机或电脑连上热点，浏览器打开本地页面上传 .bin——注意，这条通道只接受已经拆好的单应用包，不拆整包镜像（权衡后没做拆包能力）。安全就靠这个配对码：它只显示在设备自己的屏幕上，手里拿着设备、看得见屏幕的人才传得进固件。传完校验，失败就擦掉整个槽位，不留半成品。
