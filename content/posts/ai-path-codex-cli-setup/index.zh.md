...
cover:
  image: "cover.png"
  alt: "水彩风格：ChatGPT 桌面应用窗口悬浮在中央，Chat/Work/Codex 三种模式图标环绕，背后是代码滚动的终端窗口，三个几何图形代表 GPT-5.6 的 Sol/Terra/Luna 三款模型"
---

> 这是「AI 之路进阶升级指南」的番外篇。如果你还没装 Codex，这篇帮你从零跑通。

2026年7月9日，OpenAI 发布了两项重要消息：GPT-5.6 公开发布，Codex 并入 ChatGPT 桌面版。新的桌面应用有 Chat、Work、Codex 三种模式，装一个 app 就能切换。之前装过独立 Codex 桌面应用的用户直接更新就行，项目不会丢。
...
| Go | $8/月 | Terra only，扩展额度 | 比 Free 多一些，轻度够用 |
| Plus | $20/月 | Sol/Terra/Luna，更高额度 | 个人日常推荐 |
| Pro | $200/月 | 同上，更高额度，ultra 可用 | 深度使用 |
...
![](illustration.png)
*ChatGPT 桌面版的三模式切换：Chat、Work、Codex，装一个 app 就能在三种能力之间切换*

---

## 第一部分：ChatGPT 账号注册与付费

已有 ChatGPT Plus 或更高等级付费账号，直接进入安装部分。
...
OpenAI 网页端支持国际信用卡（Visa、Mastercard、American Express、Discover），不支持 PayPal。手机端另有渠道：

- **iOS**：通过 App Store 订阅，可用 Apple Pay
- **Android**：通过 Google Play 订阅，可用 Google Pay
...
付款完成之后账号就升级为 Plus 了，Codex 的额度也包含在内。

---

## 第二部分：安装 Codex——桌面版（推荐）

ChatGPT 桌面版是现在用 Codex 更直接的方式。
...
如果你习惯用之前的 ChatGPT 桌面版，旧版本已更名为 ChatGPT Classic，同样可以切换。Codex 模式能设为默认视图，macOS 上也能保留独立 Codex 图标。
