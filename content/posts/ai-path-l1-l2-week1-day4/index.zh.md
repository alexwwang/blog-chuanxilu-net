...
**加了系统提示词**。`messages` 列表里增加了 `"role": "system"` 的消息，用来给 AI 设定身份和任务规则。系统提示词按输入 tokens 正常计费（没有额外费用）。虽然占用了一点对话消息长度，但能保证输出结果质量更好、更稳定，避免返工的浪费。`temperature=0.3` 让总结结果更一致，`max_tokens=500` 控制输出长度。
...
*脚本跑不起来？把错误信息截图发到读者群（不要贴 API Key）。最常见的原因：`.env` 文件位置不对、文件夹名写错、网络超时。*
