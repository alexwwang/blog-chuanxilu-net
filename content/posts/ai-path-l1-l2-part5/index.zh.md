...
> 这是「AI 之路进阶升级指南」L2 的补充篇，讲不懂编码的人怎么用 AI 写代码。

L2 还差最后一步：*让 AI 写出真正能用的代码。*

**你怎么让 AI 写出真正能用的代码？**

不是跑通的 demo。是能在你的项目里直接跑的脚本。

一个不懂编码的人，面对 AI 生成的代码，最大的痛点不是"怎么写代码"，而是三个问题：
...
**R（Role / 角色）**：让 AI 扮演什么角色。"你是一个 Python 脚本工程师"和"你是一个经验丰富的后端开发者"，输出的代码质量不同。前者给基础脚本，后者会给模块化、带注释的版本。
...
**F（Format / 格式）**：代码的输出格式。函数签名、模块结构、注释风格。这些不说，AI 给的代码可能能用，但不符合你的规范。

**C（Constraints / 约束）**：和 GCO 的 Constraints 一致，但代码任务有更具体的约束类型：
- 环境约束：只能用标准库，还是可以用第三方？
- 性能约束：处理多少数据？有没有时间要求？
- 安全约束：不能写文件、不能联网、不能执行系统命令
- 风格约束：命名规范、注释要求

五个要素递进。Role 定基调，Context 定边界，Task 定目标，Format 定结构，Constraints 定底线。缺 Context 是代码问题最常见的根因——AI 不知道你在用什么环境，写的代码跑不起来。
...
- **业务与接口级别的格式（非程序员也能掌控）**：数据以什么形式传进来、以什么格式交出去、函数接受哪些参数名。这本质上是**业务规则**，不是技术门槛。
...
TDD Pipeline 就是答案。它是一套基于 RCTFC 的多阶段工作流，指导 AI 把业务需求转化为可交付代码。每个阶段有独立的输入、输出和审核机制，阶段之间通过固定格式的文件传递信息。
...
每个阶段结束都过一轮独立审核——不是 AI 自评，是换个视角专门检查。连续两轮没有 C/H/M 级问题才进入下一阶段。
...
后面的阶段——技术方案、测试用例、代码实现——都是 AI 根据你的输入自主推导的。**注意：这里的"写测试代码"不是让你去敲 `assert`，而是让 AI 自己先写出用来拷问它自己的检验考卷。**你不需要写一行代码，但你全程在控制方向。
...
让 AI 写代码不只是描述问题和验证结果，还需要一些项目习惯：
