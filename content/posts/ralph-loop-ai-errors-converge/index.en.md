...
Map Ralph Loop review rounds onto a sequence: {a₁, a₂, a₃, ...}, where aₙ is the number of issues found in round n. The goal is for this sequence to converge to zero.

One clean round means aₙ = 0. This does not prove convergence: aₙ₊₁ could be positive, and the sequence is still oscillating. Two consecutive clean rounds means aₙ = aₙ₊₁ = 0. The difference between consecutive terms is zero. This captures the core intuition of the Cauchy condition: the check is not the distance between aₙ and the limit (the limit is unknown); it is the distance between terms. And because issue counts are non-negative integers, two consecutive zeros make further deviation impossible without an external change; this effectively satisfies the Cauchy condition.
...
The Coroutine-O data above shows single-review convergence. Here is a different project, a Python data backfill service[6], which ran the full tdd-pipeline across all five phases:
