---
title: "Context Rot: An Overlooked Problem in AI Coding"
...
Looking back at my data, that 992-message session on "reflection skill dual-platform blog series planning" contained three logical units: writing the first blog post, writing the second blog post, series planning discussion. Three serialized posts needed to echo each other, so to maintain coherence, they weren't split up. The 9 compactions weren't the cost—they were an investment in active context management. In a long session, if you don't compact proactively, context rot will unconsciously erode the model's performance. Those 9 compactions actually show: keeping them together is fine, but it must be paired with frequent proactive compacting. Later serialized posts became independent sessions—the fourth post only had 542 messages and 3 compactions, less than half the total of the first three.

This is a tradeoff, not an either-or choice. Balancing coherence and context pressure needs to be judged based on the specific situation. My experience is: **if multiple tasks have strong dependency relationships—where the output quality of subsequent tasks directly depends on the context of earlier tasks—keep them in the same session, but pair it with proactive compact management.** Conversely, if you're just "doing it on the side," don't be lazy—start a new session.

One rule of thumb: when you find yourself repeatedly reminding the model in conversation "we're doing X now, not Y," you should have started a new session long ago.
...
OpenCode has similar layering. Its main config file is `AGENTS.md` (project root or `~/.config/opencode/AGENTS.md`), loaded every session, so only high-density core instructions go there. If a project has `AGENTS.md`, it loads preferentially over `CLAUDE.md`—the latter is just a Claude Code compatibility mode fallback. Skills load on-demand: agents see the description then decide whether to invoke; a skill can have hundreds of lines of detailed protocol but only about 100 tokens of description permanently resident. This layering itself is a practice in context management—matching loading cost with information density.
...
But there are fuzzy times too. New session or compact, the boundary isn't always clear. As time invested on the task increases and understanding of the problem deepens, what was initially thought of as "one big task" might be re-decomposed into several independent tasks; what was initially thought of as "independent tasks" might be discovered to have implicit dependencies. Subtask division changes, so the choice between compact and split session must adjust too. That "reflection skill dual-platform blog series planning" session is an example: three serialized posts belong to the same big task, but each post's writing is itself an independent subtask. If three posts have strong coherence requirements, keeping them in one session can guarantee the model "remembers" the style and conventions from earlier. But the cost is higher compaction frequency and greater context pressure. My actual practice was: first three together, subsequent independent sessions—accumulated enough early conventions to work independently of the main session, while avoiding a single session being too heavy.
