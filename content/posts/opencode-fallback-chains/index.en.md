...
- **Sisyphus and Hephaestus share the same primary model, different variants**. Sisyphus (orchestration) uses high reasoning, Hephaestus (execution) medium is enough. Both degrade to the same model on provider-b; the backup provider doesn't need to distinguish variant.
...
- **quick category's fallback is model-z → model-x**. Weak model down → upgrade to strong model, not degrade. This direction is counter-intuitive but reasonable: quick tasks use a small model, down means provider has issues, switching to same provider's strong model might recover (different models use different service clusters).
...
Both systems can configure dedicated analysis agents (like `oracle-ds4f` and `oracle-ds4p`). They don't participate in automatic fallback; they're only used when explicitly requested. The fallback chain is self-retry: same model repeated 4 times, achieving 5 attempts.
