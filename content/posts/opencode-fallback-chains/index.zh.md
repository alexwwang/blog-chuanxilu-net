...
tags: ["AI", "opencode", "agent", "fallback", "oh-my-openagent", "gpt", "chatgbt"]
categories: ["AI 实践"]
toc: true
...
我自己的触发场景：主力 provider 有每 5 小时的配额限制，用完返回 429。不配 fallback 的话，配额耗尽直接报错，整个 session 中断。开了 runtime_fallback，配额用完自动切到备选 provider 继续干活，无感切换。

## omo 的 Fallback 架构

omo 的模型解析是一个 **五层管线**[1]，按优先级从高到低：

1. **Override**：用户通过 UI 显式选择的模型，直接返回，跳过所有后续层
2. **Category Default**：agent category 配置的默认模型，带模糊匹配[4]（`model-alpha` 能匹配到 `provider-e/model-alpha`）
3. **User Fallback Models**：用户在配置里写的 `fallback_models`，逐个尝试直到找到在线的
4. **Hardcoded Chain**：omo 内置的 per-agent 和 per-category 硬编码链[5]，跨 provider 匹配
5. **System Default**：所有层都失败时的最终兜底（`opencode/gpt-5-nano`）

关键点：用户的 `fallback_models` 不是替代硬编码链，而是**覆盖**——用户链用完了，还有硬编码链兜底。

`fallback_models` 支持四种写法[2]：
...
omo 还有**模糊匹配**能力[4]：模型名称会被规范化（小写、版本号分隔符统一），然后做子串匹配。`model-alpha` 能匹配到 `provider-e/model-alpha`，甚至不同 provider 之间的变体也能对应上。

![omo 五层 Fallback 管线架构](illustration-1.png)

## omo 的 Fallback 模式与场景

**模式一：零配置——纯靠硬编码链**

只设 primary model，不写 `fallback_models`，不开 `runtime_fallback`。纯靠内置硬编码链，omo 会自动寻找可用模型。
...
用对象格式给不同备选配不同参数，并开启运行时 fallback：
...
"fallback": {
  ...
  "timeoutMs": 15000         // 单次调用超时（默认 15s）[16]
}
```
...
**严格 Agent 隔离**[9]：每个 agent 只用自己的链。explorer 没配链，就绝对不会拿到 orchestrator 的强模型。每个 session 维护一个 tried-set[10]，记录已尝试过的模型，不会回环重试。链用完了，session 保持失败状态，不会偷偷降级。

**空响应重试**（`retry_on_empty`）[11]：模型返回了 0 token 的空内容，oms 也当故障处理。这对 council 评审场景特别有用——弱模型偶尔生成空响应，自动重试比手动重发省心。

**内联优先链**（Model Array 语法）[12]：除了 `fallback.chains`，oms 还支持在 agent 配置里直接写模型数组：
...
![omo vs oms Fallback 机制对比](illustration-2.png)
...
4. **别在 fallback 链里放专用模型**。带自定义重试逻辑的专用分析 agent（如 `oracle-ds4f`/`oracle-ds4p`）有自己的重试机制；把它们放进跨模型 fallback 链会干扰自动降级。
...
1. [omo 五层管线](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/shared/model-resolution-pipeline.ts)
2. [omo fallback_models 格式](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/config/schema/fallback-models.ts)
3. [omo runtime_fallback 配置](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/config/schema/runtime-fallback.ts)
4. [omo 模糊匹配](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/shared/model-availability.ts) · [模型名称规范化](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/shared/model-normalization.ts)
5. [omo 硬编码链](https://github.com/code-yeongyu/oh-my-openagent/blob/v4.7.5/src/shared/model-requirements.ts)
6. [oms 两层架构 + config() hook](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/index.ts)
...
8. [oms 事件监听器](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/hooks/foreground-fallback/index.ts)（`message.updated`、`session.error`、`session.status`）
9. [oms 严格 Agent 隔离](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/hooks/foreground-fallback/index.ts)（`resolveChain`）
...
12. [oms 内联优先链（Model Array）](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/config/schema.ts)
13. [oms 链合并逻辑（effectiveArrays）](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/index.ts)
14. [oms preset 系统](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/tools/preset-manager.ts)
15. [oms preset 联动（链重建）](https://github.com/alvinunreal/oh-my-opencode-slim/blob/v1.1.1/src/index.ts)
