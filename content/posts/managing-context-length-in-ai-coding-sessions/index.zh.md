...
tags: ["AI", "agent", "上下文管理", "context rot", "opencode", "claude-code", "claude"]
categories: ["AI 实践"]
cover:
  image: "cover.png"
  relative: true
...
回头看我的数据，也能印证这一点。"反思技能双平台博客连载规划"那个 992 条消息的 session，包含了三个逻辑单元：第一篇博客的写作、第二篇博客的写作、系列规划讨论。按理应该拆成三个 session。但实际没有拆——因为三篇连载需要连贯性，放在一个 session 里模型能"记住"前面的风格和约定。这涉及 compact 和拆 session 的边界判断，后面第五条会展开讨论。

**核心判断：如果新任务和当前任务不在同一个逻辑单元里，开新 session。** 如果多个任务之间有强依赖关系——后续任务的输出质量直接取决于前面任务的上下文——可以放在同一个 session 里，但要配合主动 compact 管理。同一个功能的开发和测试可以在一个 session，但功能 A 的开发和功能 B 的设计不应该。反之，如果只是"顺手做了"，不要偷懒，开新 session。
...
OpenCode 的分层类似：skill 按需加载，描述常驻但完整内容只在调用时才进入上下文；MCP server 则在启动时注册完整 schema，加载成本更高。两者的上下文开销差异，正好对应了"用不到的不加载"这个原则。
...
但也有模糊的时候。开新 session 还是 compact，边界并不总是清晰的。随着在任务上投入的时间增加，对问题理解的加深，最初认为的"一个大任务"可能被重新拆解成几个独立任务，最初以为的"独立任务"可能发现彼此有隐含依赖。子任务的划分会变，compact 和拆 session 的选择也要跟着调整。

"反思技能双平台博客连载规划"那个 session 是典型的灰色地带。992 条消息、三个逻辑单元——第一篇博客的写作、第二篇博客的写作、系列规划讨论——按理应该拆成三个 session。但三篇连载需要连贯性，放在一起模型能"记住"前面的风格和约定，所以没有拆。代价是 9 次 compaction——这不是代价，是主动管理上下文的投入。长 session 里如果不主动 compact，上下文腐烂会在不知不觉中侵蚀模型的表现。后续的连载就独立 session 了——第四篇只有 542 条消息、3 次 compaction，不到前三篇总量的一半。既积累了足够的前期约定可以脱离主 session 工作，又避免了单 session 过重的问题。
