...
这不只是 V4 实验的结论。Anthropic "Teaching Claude Why" 研究[1] 的发现是一样的：教原则（行为准则文档、伦理推理）比教行为（正确答案示例）效果好了约 7 倍。我们在 prompt 层面复现了同样的模式。
...
值得提一句：这个方向不止 Anthropic 一家在说。Wang 等人[2]在经验表示的研究中独立得出了高度一致的结论——他们用 4,590 次受控实验证明，紧凑的控制导向表示（Gene，~230 tokens）比完整的文档包（Skill，~2,500 tokens）效果更好（Gene 为 +3.0pp，而 Skill 为 -1.1pp），而且给紧凑表示加回文档材料（包括示例和 API 备注）反而削弱效果。这和我们的发现完全对应：少即是多，多加东西不一定更好。
