...
Historical entries within the KV cache are append-only and unmodifiable, meaning past output decisions remain locked in context. Once a round's Goal block is generated, it's frozen. R2's "Fact-Gather+Precision(1)" and R1's "Fact-Gather(1) → Precision(1)" coexist in context. When the model needs to know "what's the correct approach," attention must choose between two contradictory versions. Softmax doesn't do precise selection. It distributes probability across all similar tokens. If R2's version is more recent and appears more frequently, it gets higher attention weight, and the model is more likely to follow it.
