...
![聊天窗口与 API 的区别](illustration-chat-vs-api.jpeg "左：聊天窗口——你手动一个个操作；右：API——程序自动批量处理")

**怎么练习**：现在还不需要写代码，只需要建立直觉。后面我会给你完整的代码，你复制粘贴、改几个参数就能跑——不需要从零开始写。现在先回想一下你最近手动重复 3 次以上的 AI 任务——比如把 10 段文字翻译成英文、总结 5 篇文章、给 20 个客户写个性化的欢迎邮件。想一想，如果这件事能让程序自动做，能节省你多少时间？这个场景就是你接下来学习 API 的目标。
...
![Token 计费示意图](illustration-token-billing.jpeg "输入和输出分别计费——你的问题和 AI 的回复各算各的")
